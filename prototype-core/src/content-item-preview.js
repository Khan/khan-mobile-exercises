...
const ContentItemPreview = React.createClass({
	propTypes: {
	...
	render() {
		...
		return <div className="exercise-preview" style={{
			...
			<ExercisePreviewDescription
				name={this.props.contentItem.container.name}
				id={this.props.contentItem.container.key_id}
				/>
			</div>
...
import {setupForExercises, setupForArticle} from "../../bridge/src/setup"
const testPerseusItem = require("../../bridge/data/perseus-exercise.json");
...
const MobilePreview = React.createClass({
  ...
  _renderIframe() {
		const iframe = ReactDOM.findDOMNode(this);
    setupForExercises(iframe).then(({jsExerciseAPI, iframe}) => {
			if (!iframe.parentNode) {
				return;
			}
			const {container, assessment_item} = this.props.contentItem;
